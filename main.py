...
with center_col:
        ...
        if st.button("▶ NLLB 번역 시작") and files:
            ...
            with zipfile.ZipFile(zip_buf, "w") as z:
                for idx, f in enumerate(files, 1):
                    ...
                    out = trans_nllb.translate(
                        rows, tok, mdl, status, f.name, idx, len(files)
                    )
                    z.writestr(
                        f.name,
                        utils.build_srt([[r[0], r[1], t] for r, t in zip(rows, out)])
                    )
    ...
    # ======================
    with tabs[1]:
        st.caption("Gemini Flash 전용 · 언어 자동 감지")
        ...
        if st.button("▶ Gemini 번역 시작") and files:
            if not GEMINI_API_KEY:
                ...
                model_name = "gemini-2.0-flash"  # 🔥 고정
                status = st.empty()
                zip_buf = io.BytesIO()
